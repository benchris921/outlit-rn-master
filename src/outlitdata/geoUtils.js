/**
 * Useful constants and functions that support the GeoQuery class;
 *
 * @author GeoFire Team
 */
...
export const g_MAXIMUM_BITS_PRECISION = 22 * g_BITS_PER_CHAR;
...
function validateLocation(location) {
  ...
  if (!Array.isArray(location)) {
    ...
    else if (typeof longitude !== 'number' || Number.isNaN(longitude)) {
...
function validateGeohash(geohash) {
  ...
  if (typeof geohash !== 'string') {
    ...
    for (let i = 0, length = geohash.length; i < length; ++i) {
...
function decodeGeoFireObject(geoFireObj) {
  if (geoFireObj !== null && geoFireObj.hasOwnProperty('loc') && Array.isArray(geoFireObj.loc) && geoFireObj.loc.length === 2) {
    return geoFireObj.loc;
  }
  // else
...
function getKey(doc) {
  let key;
  if (typeof doc.id === 'function') {
    key = doc.id();
  } else if (typeof doc.id === 'string' || doc.id === null) {
    key = doc.id;
  } else {
    key = doc.id;
  }
...
export {
  distance, validateLocation, validateCriteria, degreesToRadians, encodeGeohash,
  validateGeohash, geohashQueries, getKey, decodeGeoFireObject, encodeGeoFireObject,
};
